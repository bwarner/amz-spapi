...
export function createBedrockProvider(config: AIProviderConfig): AIProvider {
  ...
  return {
    ...
    languageModel(tier: ModelTier = 'default'): LanguageModelV2 {
      return getBedrock().languageModel(models[tier]) as unknown as LanguageModelV2;
    },

    embeddingModel(): EmbeddingModelV2<string> {
      return getBedrock().embeddingModel(embeddingModelId) as unknown as EmbeddingModelV2<string>;
    }
  };
}
